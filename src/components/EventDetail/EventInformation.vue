...
onMounted(() => {
  meetingDescription.value = props.meeting.description
})

const changeDescription = () => {
  emits('updateDescription', meetingDescription.value)
  ...
  <div :class="$style.container">
    <div :class="$style.informationContainer">
      <div :class="$style.titleContainer">
        <p :class="$style.meetingTitle">{{ meeting.title }}</p>
      ...
      <p :class="$style.startedAt">
        {{ `${formatDateTime(new Date(meeting.startedAt))}~` }}
      </p>
      <div
    ...
    <a
      :href="`https://youtu.be/${meeting.videoId}`"
      target="_blank"
      :class="$style.thumbnailImage"
    >
      <img :src="meeting.thumbnail" :class="$style.thumbnailImage" />
    </a>
  </div>
</template>
